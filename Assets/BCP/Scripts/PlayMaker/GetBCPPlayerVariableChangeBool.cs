...
public class GetBCPPlayerVariableChangeBool : FsmStateAction
{
    [RequiredField]
    ...
    [RequiredField]
    [UIHint(UIHint.Variable)]
    [Tooltip("The variable to receive the value of the specified MPF player variable (bool)")]
    public FsmBool value;

    [UIHint(UIHint.Variable)]
    [Tooltip("The variable to receive the previous value of the specified MPF player variable (bool)")]
    public FsmBool previousValue;

    [UIHint(UIHint.Variable)]
    [Tooltip("The variable to receive the change in value of the specified MPF player variable (bool)")]
    public FsmBool change;
    ...
    public void PlayerVariable(object sender, PlayerVariableMessageEventArgs e)
    {
        ...
        if (!String.IsNullOrEmpty(playerVariableName) && e.Name == playerVariableName)
        {
            ...
            if (!value.IsNone)
                value.Value = e.Value.AsBool;

            if (!previousValue.IsNone)
                previousValue.Value = e.PreviousValue.AsBool;

            if (!change.IsNone)
                change.Value = e.Change.AsBool;
