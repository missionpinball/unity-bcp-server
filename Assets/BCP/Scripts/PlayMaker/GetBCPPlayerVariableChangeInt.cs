...
public class GetBCPPlayerVariableChangeInt : FsmStateAction
{
    [RequiredField]
    ...
    [RequiredField]
    [UIHint(UIHint.Variable)]
    [Tooltip("The variable to receive the value of the specified MPF player variable (int)")]
    public FsmInt value;

    [UIHint(UIHint.Variable)]
    [Tooltip("The variable to receive the previous value of the specified MPF player variable (int)")]
    public FsmInt previousValue;

    [UIHint(UIHint.Variable)]
    [Tooltip("The variable to receive the change in value of the specified MPF player variable (int)")]
    ...
    public void PlayerVariable(object sender, PlayerVariableMessageEventArgs e)
    {
        ...
        if (!String.IsNullOrEmpty(playerVariableName) && e.Name == playerVariableName)
        {
            ...
            if (!value.IsNone)
                value.Value = e.Value.AsInt;

            if (!previousValue.IsNone)
                previousValue.Value = e.PreviousValue.AsInt;

            if (!change.IsNone)
                change.Value = e.Change.AsInt;
