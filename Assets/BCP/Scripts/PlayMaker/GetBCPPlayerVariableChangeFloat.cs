...
public class GetBCPPlayerVariableChangeFloat : FsmStateAction
{
    [RequiredField]
    ...
    [RequiredField]
    [UIHint(UIHint.Variable)]
    [Tooltip("The variable to receive the value of the specified MPF player variable (float)")]
    public FsmFloat value;

    [UIHint(UIHint.Variable)]
    [Tooltip("The variable to receive the previous value of the specified MPF player variable (float)")]
    public FsmFloat previousValue;

    [UIHint(UIHint.Variable)]
    [Tooltip("The variable to receive the change in value of the specified MPF player variable (float)")]
    public FsmFloat change;
    ...
    public void PlayerVariable(object sender, PlayerVariableMessageEventArgs e)
    {
        ...
        if (!String.IsNullOrEmpty(playerVariableName) && e.Name == playerVariableName)
        {
            ...
            if (!value.IsNone)
                value.Value = e.Value.AsFloat;

            if (!previousValue.IsNone)
                previousValue.Value = e.PreviousValue.AsFloat;

            if (!change.IsNone)
                change.Value = e.Change.AsFloat;
